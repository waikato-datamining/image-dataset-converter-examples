...
the image-dataset-converter library offers support for *variables*. 
...
* **User-defined** ones, which are supplied to the tool itself, e.g., via the
  `--variables` option of the `idc-convert` tool. The same script can
...
variables like `{INPUT_PATH}`. The following command saves the current

...
## Overlaying annotations/predictions

You can use the [add-annotation-overlay-od](https://github.com/waikato-datamining/image-dataset-converter-imgvis/blob/main/plugins/add-annotation-overlay-od.md) 
plugin for overlaying annotations/predictions of certain labels, e.g., 
ones generated by an object detection model like YOLO.

Below, only *dog* and *sheep* objects will be added as overlay:

```bash
  ...
  add-annotation-overlay-od --labels dog sheep \
  ...
``` 

## Generating video from overlays using ffmpeg

[ffmpeg](https://ffmpeg.org/) can generate videos from JPG images, e.g.,
ones with annotations overlaid (see above). Since ffmpeg requires
sequential numbers in the file names, you can rename your overlays with the
[rename](https://github.com/waikato-datamining/image-dataset-converter/blob/main/plugins/rename.md)
plugin, e.g.:
...
The command for generating a video from the enumerated images is as follows
(`-r 5` = five frames per second):

```bash
ffmpeg -r 5 -i "%d.jpg" -pix_fmt yuv420p -f mp4 video.mp4
```
